...
function renderizarPedidos() {
  ...
  const pedidos = JSON.parse(localStorage.getItem("techfood_pedidos") || "[]"); //o "[]" evita o erro de vir null, e sim mostrar a lista vazia

  if (pedidos.length === 0) {
    lista.innerHTML =
      "<li class='pedido-vazio'> Nenhum pedido ainda. acesse o " +
      "<a href='index.html'>Cardapio</a> para adicionar 😊</li>";
  ...
  pedidos.forEach(function (pedido, indice) {
    ...
    btnRemover.addEventListener("click", function () {
      ...
      lista.splice(indice, 1); //retirando a informação da lista do localStorage pelo indice
      localStorage.setItem("techfood_pedidos", JSON.stringify(lista)); //confirmando a exclusão
      renderizarPedidos();
  ...
  const totalFnt = "R$ " + total.toFixed(2).replace(".", ",");
  if (spanTotal) spanTotal.textContent = totalFnt;
  if (spanResumo) spanResumo.textContent = totalFnt;
